...
(function ($) {
    ...
    $(document).ready(function () {   
        ...
        function cover(id, index) {
            ...
            if($(cover).data('bg-type') === 'image') {
                ...
                $(cover).transition({ opacity: 1 }, 1000, 'ease', function() {
					...
					vid_w_orig = parseInt($('.cover-' + id + ' .cover-video video').attr('width'));
					vid_h_orig = parseInt($('.cover-' + id + ' .cover-video video').attr('height'));
		...
		if(!no_transitions) {
			$('.logo a, .title h1 a, .title a, .thumb a, li.menu-item a, li.page_item a, h2 a, a.more-link, .meta a, .project-panel-link, .fullscreen-cover a, .fwt-link, .secondary a, .featured a, #fullscreen-menu a, a.ce-image-link, a.cover-slider-link, a.cover-link').click(function (a) {
